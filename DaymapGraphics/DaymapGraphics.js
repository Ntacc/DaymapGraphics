...
// "moves" where the point is on the gradient to find the colour.
...
// Transferring settings from local storage to synced storage, as previous versions used local storage.
if(localStorage && !localStorage.getItem("settingsTransferred")) {
	setItem("translucentMode", localStorage.getItem("translucentMode"));
	setItem("blurAmount", localStorage.getItem("blurAmount"));
	setItem("autoAttendanceRainbow", localStorage.getItem("autoAttendanceRainbow"));
	setItem("originalLayout", localStorage.getItem("originalLayout"));
	setItem("bodyBackground", localStorage.getItem("bodyBackground"));
	localStorage.setItem("settingsTransferred", 1);
}
...
(async () => {
	...
	.then(storage => {
		...
		// Determining the element to make rainbow coloured
		...
		if(getItem("translucentMode") && parseFloat(getItem("translucentMode")) < 1) {
			...
			dark = dark == "1" ? 1 : 0;
			var translucentness = parseFloat(getItem("translucentMode"));
			for(i = 0; i < document.querySelectorAll(".card, .msg, .ditm, .Toolbar").length; i ++) {
				document.querySelectorAll(".card, .msg, .ditm, .Toolbar")[i].style.background = "rgba(" + (dark ? "37, 37, 37," : "237, 235, 233,") + translucentness + ")";
			}
			...
			for(i = 0; i < document.querySelectorAll(".hasDatepicker").length; i ++) {
				document.querySelectorAll(".hasDatepicker")[i].style.backgroundColor = "rgba(" + (dark ? "37, 37, 37," : "237, 235, 233,") + translucentness * 0.7 + ")";
			}
			...
			for(i = 0; i < document.querySelectorAll(".nav-container, .nav-user-container").length; i++) {
				document.querySelectorAll(".nav-container, .nav-user-container")[i].style.backgroundColor = "rgba(" + (dark ? "37, 37, 37," : "237, 235, 233,") + translucentness * 0.7 + ")";
			}
			window.setTimeout(() => {
			}, 510);
			window.setTimeout(() => {
